...
for novel in novels:
    try:
        cursor.execute(
            insert, (novel["name"], False, datetime.now(), "placeholder.png")
        )
        psql.commit()
    ...
    chapters = db.execute(
        f"SELECT * FROM chapters WHERE novel_id = {novel["novel_id"]}"
    )

    for chapter in chapters:
        cursor.execute(
            "INSERT INTO novel_chapter (novel_id,  title, content, num, views, date) VALUES (%s, %s, %s, %s, %s, %s)",
            (
                novel_id,
                chapter["title"],
                chapter["content"],
